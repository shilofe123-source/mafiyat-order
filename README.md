...
- **תהליך הזמנה**: בחירת מוצרים > אישור > שמירה בגיליון > שליחת PDF אוטומטית בוואטסאפ לאורי ובת חן + מייל לבת חן > שמירת PDF בתיקיית החודש בדרייב > עדכון גליון כמויות + הזמנות מעודכן
- **כשרות**: מהדרין רבנות קרני שומרון
- **צ'אט AI**: ייעוץ אוטומטי ללקוחות — שאלות מנחות לפני המלצות, המלצות לפי סוג אירוע, עונה, כמות אורחים. הסוכן יכול למלא פרטי לקוח, להוסיף/להסיר מוצרים, ולקבל תמונות ו-PDF של הזמנות
...
| `information.js` | מאגר ידע לצ'אט — מידע כללי, שאלות נפוצות, מחירון, סלטים, דוגמאות הזמנות |
| `menu.html` | דף תפריט — 33 מוצרים עם סינון לפי קטגוריה |
| `gallery.html` | גלריית תמונות מוצרים |
| `about.html` | דף אודות — סיפור המאפייה, כשרות, ערכים |
| `images/` | 33 תמונות מוצרים דחוסות (600px, ~400-700KB) |

## אינטגרציות

- **Google Sheets** — שמירת הזמנות אוטומטית + גיליון "סיכום" + גיליון "כמויות" (מצרכים להכנה) + גיליון "הזמנות מעודכן" (חלון מתגלגל חודשי) + גיליון "feedback" + גיליון "errors"
- **Email** — שליחת מייל אוטומטית לבת חן עם PDF מצורף בכל הזמנה חדשה
